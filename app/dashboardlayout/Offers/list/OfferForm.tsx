...
export default function OfferForm({ open, onClose, onSubmit, editingOffer }: OfferFormProps) {
  ...
  useEffect(() => {
    if (editingOffer) setFormData(editingOffer);
    else
      setFormData({
        montlyPrice: 0,
        discountPercentage: 0,
  ...
  const handleChange = (field: keyof Offer, value: number) => {
      ...
      >
        ...
        <Box display="flex" flexDirection="column" gap={2}>
          <TextField
            label="Prix mensuel"
            type="number"
            value={formData.montlyPrice ?? ''}
            onChange={(e) => handleChange('montlyPrice', Number(e.target.value))}
            fullWidth
            variant="outlined"
          ...
          <TextField
            label="Jours ajoutés"
            type="number"
            value={formData.addedDyas ?? ''}
            onChange={(e) => handleChange('addedDyas', Number(e.target.value))}
            fullWidth
            variant="outlined"
            InputProps={{ sx: { borderRadius: 2 } }}
          />
